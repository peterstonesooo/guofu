...
class UserController extends AuthController
{
    ...
    {
        $templatePath = 'storage/teamplate/template.xlsx';

        // 使用绝对路径检查文件是否存在
        $absolutePath = root_path() . $templatePath;

        // 下载文件
        if (file_exists($absolutePath)) {
            $filename = 'batch_deposit_template.xlsx';
            ...
            return out(null, 10001, '模板文件不存在: ' . $absolutePath);
        }
    }
